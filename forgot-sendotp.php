...
if($email != '')
{
    ...
    else
    {
          ...
          if($check1 == 1) {
    $sql1 = "INSERT INTO `forgot_otp`(`email`, `otp`) VALUES (?, ?)";
    $stmt1 = mysqli_prepare($con, $sql1);
    mysqli_stmt_bind_param($stmt1, "si", $email, $otp1);
    ...
    // Use the same template as profile_aboutme.php
    $customHtml = "
        <p style='font-size:15px;'>Dear $name,</p>
        <p style='font-size:15px;'>Your One Time Password (OTP) for password reset is: <strong>$otp1</strong></p>
        <p style='font-size:15px;'>This OTP is valid for 10 minutes.</p>
        <p style='font-size:15px;'>Please do not share this OTP with anyone.</p>
        <p style='font-size:12px; color:#666;'>If you didn't request this, please ignore this email.</p>
    ";
    
    // Use the SAME email layout function
    $mailContent = getEmailLayout($customHtml);
    
    // Use the SAME configuration as profile_aboutme.php
    $curl1 = curl_init();
    ...
    curl_setopt_array($curl1, array(
        ...
        CURLOPT_POSTFIELDS => array(
            'to' => $to,
            'from' => 'info@noreplies.co.in', // Same as working
            'from_name' => 'Desi Rishta',
            'subject' => $subject,
            'body' => $mailContent, // Use templated content
            'token' => '74765968c67007219b197f4d9aafb4e2' // Same token as working
        ),
    ));
    ...
    // Debug logging
    error_log("Forgot Password Email Response: " . $response1);
    ...
    if($httpCode == 200) {
        ...
        <form action="forgot-verifyotp.php" method="post" id="myForm5">
            <input type="hidden" name="validemail" value="<?php echo htmlspecialchars($email); ?>">
            <input type="hidden" name="attempt" value="<?php echo isset($emattempt) ? $emattempt : "2"; ?>">
            <input type="hidden" name="display_contact" value="<?php echo htmlspecialchars($masked_email); ?>">
        </form>
        <script>
        ...
        ?>
        <form action="forgot-password.php" method="post" id="myFormError">
            <input type="hidden" name="error" value="email_failed">
            <input type="hidden" name="debug" value="<?php echo htmlspecialchars($response1); ?>">
        </form>
        <script>
            alert('Failed to send OTP email. Please try again.');
            document.getElementById("myFormError").submit();
        </script>
        <?php
    }
} else {
