...
// 1. Fetch User Data (including name) FIRST to set the dynamic filename
$sql_fetch = "
    SELECT 
...
if (!$result || mysqli_num_rows($result) == 0) {
    echo "Error: User data not found for export.";
    exit;
}
...
$user_name = $row['name'] ?? 'Unknown_User';
// Sanitize name for filename: replace spaces with underscores and remove non-alphanumeric characters (except - and _)
$safe_name = preg_replace('/[^A-Za-z0-9\-\_]/', '', str_replace(' ', '_', $user_name));

// 2. Set Headers for CSV Download with the new dynamic filename
$filename = $safe_name . "_" . $userid . "_" . date('Ymd_His') . ".csv";
...
// 3. Define CSV Headers (Column Names)
$header_row = array(
...
// 4. Write data row to CSV
    ...
    $data_row = array(
        $row['userid'], $row['name'], $row['email'], $row['phone'], date('Y-m-d', strtotime($row['entrydate'])), $profile_status, $row['verificationinfo'],
        // Basic Info
