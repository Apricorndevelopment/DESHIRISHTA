...
if ($lis.length > 0) {
    ...
    // Is code ko js/custom.js mein add karein

$(document).ready(function() {

    // Counter-Up Animation for Analytics
    var a = 0; // Flag taaki animation ek hi baar chale
    
    // Window scroll event
    $(window).scroll(function() {

        // Check karein ki '.ab-cont' section screen par dikh raha hai ya nahi
        var oTop = $('.ab-cont'); 
        if(oTop.length > 0) { // Check karein ki element page par hai
            
            oTop = oTop.offset().top - window.innerHeight;
            if (a == 0 && $(window).scrollTop() > oTop) {
                a = 1; // Animation ko dobara na chalne dein
                
                $('.counter').each(function() {
                    var $this = $(this),
                        countTo = $this.attr('data-count'); // HTML se data-count value lena
                    
                    $({ countNum: $this.text()}).animate({
                        countNum: countTo
                    },
                    {
                        duration: 6000, // Animation ki speed (2 seconds)
                        easing: 'swing',
                        step: function() {
                            // Number ko format karein (floor value)
                            $this.text(Math.floor(this.countNum));
                        },
                        complete: function() {
                            // Final value set karna
...
// Team Slider
$('.team-slider').slick({
    slidesToShow: 4,      // Ek baar mein 4 dikhaye
    slidesToScroll: 1,
    autoplay: true,
    autoplaySpeed: 6000,
    dots: false,          // Dots (indicator) hide karein
    arrows: true,         // Arrows (Next/Prev) show karein
    responsive: [
        {
        ...
        {
            breakpoint: 600, // Mobile par
            settings: {
                slidesToShow: 2,
            }
        },
        {
            breakpoint: 480, // Chhote mobile par
            settings: {
                slidesToShow: 1,
            }
        }
...
$(document).ready(function() {
    ...
    // ... (aapke custom.js ka baaki code yahaan ho sakta hai) ...

    // Counter-Up Animation for Analytics
    var analytics_counter_ran = 0; // Ek flag taaki animation ek hi baar chale

    $(window).scroll(function() {

        var analytic_section = $('.ab-cont'); // Hamara Analytics section
        
        // Pehle check karein ki yeh section page par hai ya nahi
        if(analytic_section.length > 0) { 
            
            // Calculate karein ki section kahan hai
            var oTop = analytic_section.offset().top - window.innerHeight;
            
            // Check karein ki user scroll karke wahan pahunch gaya hai AND animation pehle nahi chala hai
            if (analytics_counter_ran == 0 && $(window).scrollTop() > oTop) {
                
                analytics_counter_ran = 1; // Flag set karein taaki dobara na chale
    ...
    // ... (baaki ka code yahaan ho sakta hai) ...
